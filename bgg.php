...
class BGGHandler {
  ...
  public function getTopRated($threshold = 9,$pre = "<tr><td>",$post = "</td></tr>",$seperator = "</td><td>") {
    $count = 0;
    for($i = 10; (($i > 0) || ($count < $threshold)); $i--) {
      foreach($this->bggData as $cData) {
        if($cData->stats->rating->attributes()->value == $i) {
          //print($pre."<a href=\"http://boardgamegeek.com/boardgame/".$cData->attributes()->objectid."/\">".$cData->name."</a>".$seperator.$cData->stats->rating->attributes()->value.$post);
          print($pre.$cData->stats->rating->attributes()->value.$seperator."<a href=\"http://boardgamegeek.com/boardgame/".$cData->attributes()->objectid."/\">".$cData->name."</a>".$post);
          $count++;
        }
        if($count >= $threshold) {
          break;
